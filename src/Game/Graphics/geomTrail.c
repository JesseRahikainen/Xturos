...
static void swap( void )
{
	for( size_t i = 0; i < sb_Count( sbGeomTrails ); ++i ) {
		...
		if( ( sb_Count( sbGeomTrails[i].sbTrailPoints ) <= 0 ) ||
			...
			addNewTrailPoint( &(sbGeomTrails[i]), &(sbGeomTrails[i].currOriginPos ), sbGeomTrails[i].time + ( lastPhysicsTick * amtTickElapsed ) );
		}
	}
}
...
static void physicsTick( float dt )
{
	...
	for( size_t i = 0; i < sb_Count( sbGeomTrails ); ++i ) {
		if( !sbGeomTrails[i].inUse ) continue;
		trailPhysicsTick( &(sbGeomTrails[i]), dt * timeScale );
	}
}
