...
// JTR - Changed the file name to avoid conflicts, made it work the SDL_RWops instead of FILE*,
//  and set it to use my memory manager. Should probably do more clean up, and set it up so
//  there are custom memory allocation functions.
...
static void *setup_malloc(vorb *f, int sz)
{
   ...
   return sz ? mem_Allocate(sz) : NULL;
}
...
void inverse_mdct_slow(float *buffer, int n)
{
   int i,j;
   int n2 = n >> 1;
   float *x = (float *) mem_Allocate(sizeof(*x) * n2);
...
int stb_vorbis_decode_filename(const char *filename, int *channels, int *sample_rate, short **output)
{
   ...
   *channels = v->channels;
   ...
   offset = data_len = 0;
   total = limit;
   data = (short *) mem_Allocate(total * sizeof(*data));
   if (data == NULL) {
      stb_vorbis_close(v);
      return -2;
   }
   for (;;) {
      ...
      if (offset + limit > total) {
         short *data2;
         total *= 2;
         data2 = (short *) mem_Resize(data, total * sizeof(*data));
         if (data2 == NULL) {
            mem_Release(data);
            stb_vorbis_close(v);
            return -2;
